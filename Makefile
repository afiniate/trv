...
metadata:
	$(TRV) build make-meta --name $(NAME) --target-dir $(LIB_DIR) --root-file Makefile --semver $(SEMVER) --description-file '$(DESC_FILE)' $(MOD_DEPS)

# This is only used to help during local opam package
# development
opam: build
	$(TRV) opam make-opam --target-dir $(CURDIR) --name $(NAME) --semver $(SEMVER) --homepage $(HOMEPAGE) --dev-repo $(DEV_REPO) --lib-dir $(LIB_DIR) --license $(LICENSE) --author $(AUTHOR) --maintainer $(AUTHOR) --bug-reports $(BUG_REPORTS) --build-cmd "make" --install-cmd 'make "install" "PREFIX=%{prefix}%" "SEMVER=%{aws_async:version}%"' --remove-cmd 'make "remove" "PREFIX=%{prefix}%"' $(BUILD_MOD_DEPS) $(MOD_DEPS)
